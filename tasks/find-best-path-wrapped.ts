...
task(
  "find-best-path-wrapped",
  "Queries MonagWrapRouter for the best path",
  async (args, hre) => {
...
class Quoter {
  ...
  async getBestOffer(swapArgs: any) {
    ...
    const wrapper = await this.hre.ethers.getContractAt(
      "contracts/MonagWrapper.sol:MonagWrapper",
      swapArgs.wrapper
    );
    const offer =
      swapArgs.tokenIn == (await wrapper.getWrappedToken())
        ? await MonagRouter.unwrapAndFindBestPath(
            swapArgs.amountIn,
            swapArgs.tokenOut,
            swapArgs.wrapper,
            swapArgs.maxHops
          )
        : await MonagRouter.findBestPathAndWrap(
            swapArgs.amountIn,
            swapArgs.tokenIn,
            swapArgs.wrapper,
            swapArgs.maxHops
  ...
  async getTokenSymbols(tokens: any) {
    return Promise.all(tokens.map((t: any) => this.getSymbol(t)));
  }
