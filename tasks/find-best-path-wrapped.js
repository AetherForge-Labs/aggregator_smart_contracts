...
(0, config_1.task)("find-best-path-wrapped", "Queries MonagWrapRouter for the best path", async (args, hre) => {
...
class Quoter {
    ...
    async getBestOffer(swapArgs) {
        const MonagRouter = await this.hre.ethers.getContract("MonagWrapRouter");
        console.log(swapArgs.wrapper);
        const wrapper = await this.hre.ethers.getContractAt("contracts/MonagWrapper.sol:MonagWrapper", swapArgs.wrapper);
        const offer = swapArgs.tokenIn == (await wrapper.getWrappedToken())
            ? await MonagRouter.unwrapAndFindBestPath(swapArgs.amountIn, swapArgs.tokenOut, swapArgs.wrapper, swapArgs.maxHops)
            : await MonagRouter.findBestPathAndWrap(swapArgs.amountIn, swapArgs.tokenIn, swapArgs.wrapper, swapArgs.maxHops);
        return offer;
    }
